...
class Storiesloader:
    ...
    def create_user_folder(self, target) -> None:
        ...
        Create a downloaded account folder to save the stories
        ...
        self.user_main_folder = f'{self.target_username}_stories_{self.target_userid}'
...
-------------------------------------------------------------------\n''')
    ...
    def download_user_stories(self):
        """
        Summary of the Function:
        Download the stories of the targeted account (Default: All Highlights)
...
def main():
    storiesdl = Storiesloader()
    def download_run():
        """
        Summary of the Function:
        Download the stories of the targeted account (Default: All Stories)

        """

        for target in storiesdl.target_list:
            profile_dict = storiesdl.get_user_profile(target)

            if profile_dict != False: # profile is existing on instagram = True
                storiesdl.create_user_folder(target)

                try:
                    storiesdl.download_user_stories()
